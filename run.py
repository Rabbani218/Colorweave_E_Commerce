...
def maybe_apply_migrations(app) -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).info(
            "Applied database migrations (upgrade)",
        )
    except Exception as exc:  # pragma: no cover - best-effort safeguard
        logging.getLogger(__name__).warning("Skipping migrations: %s", exc)


def maybe_warm_ai(app) -> None:
    ...
    try:
        with app.app_context():
            ...
            try:
                ...
                logging.getLogger(__name__).info(
                    "Embedding index warmed: %d items",
                    len(idx.ids),
                )
            except Exception as exc:  # pragma: no cover - best-effort safeguard
                logging.getLogger(__name__).warning(
                    "Embedding warm failed: %s",
                    exc,
                )
            ...
            try:
                ...
                vision = VisionIndexer(
                    static_folder=static_folder,
                    persist_dir=persist_dir,
                )
                vision.build_index(force=False)
                logging.getLogger(__name__).info(
                    "Vision index warmed: %d items",
                    len(vision.ids),
                )
            except Exception as exc:  # pragma: no cover - best-effort safeguard
                logging.getLogger(__name__).warning(
                    "Vision warm failed: %s",
                    exc,
                )
    except Exception as exc:  # pragma: no cover - best-effort safeguard
        logging.getLogger(__name__).warning(
            "AI warm sequence skipped: %s",
            exc,
        )
...
def main() -> None:
    ...
    maybe_apply_migrations(flask_app)
    maybe_warm_ai(flask_app)

    host = (
        os.environ.get("HOST")
        or os.environ.get("FLASK_RUN_HOST")
        or "127.0.0.1"
    )
    port = int(
        os.environ.get("PORT")
    ...
    if _bool_env("USE_NGROK", False):
        with suppress(Exception):
            ...
            region = os.environ.get("NGROK_REGION") or "ap"
            ...
            logging.getLogger(__name__).info(
                "ngrok tunnel active: %s -> http://%s:%s",
                public_url,
                host,
                port,
            )
    ...
    logging.getLogger(__name__).info(
        "Starting ColorWeave on http://%s:%s (debug=%s)",
        host,
        port,
        debug,
    )
